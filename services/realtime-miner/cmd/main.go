...
func main() {
	...
	historicalJobChan := make(chan uint64, 1000)
	blockCollector := collectorLib.NewBlockCollector(providerClient, 20, logger)

	// Инициализация RealtimeCollector
	realtimeCollector := collector.NewRealtimeCollector(blockCollector, historicalJobChan)
	...
	go func() {
        for {
            select {
            case blockNum := <-historicalJobChan:
                logger.Warnf("Historical job received block %d, but no Historical Miner is listening. Dropping job.", blockNum)
            case <-ctx.Done():
                logger.Debug("Historical job handler stopping.")
                return
            }
        }
    }()
